...
| **Call Audio: pipewiresrc → autoaudiosrc** | WebRTC call audio (`webrtc_call_manager.vala`) uses `pipewiresrc` directly for audio capture. `pipewiresrc` defaults to video stream type — using it for audio is fragile. Replace with `autoaudiosrc`. Requires GStreamer interop testing (latency, echo cancellation, device selection, Conversations/Monal compatibility). | **PLANNED** — evaluate for v1.2.x |
| **Unified Source Selection** | Long-term: all pipelines (video messages, WebRTC calls, legacy Jingle) use consistent source selection: `Gst.DeviceMonitor` → `create_element()` (preferred) or `autovideosrc`/`autoaudiosrc` (fallback). Never use `pipewiresrc`/`v4l2src` directly. | CONCEPT — v1.3.x |
