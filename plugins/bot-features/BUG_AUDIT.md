...
**Status:** ⚠️ Mitigation implementiert — Deutliche Warnung wird jetzt angezeigt, dass alle Benutzer betroffen sind. Grundproblem besteht weiterhin: ejabberd bietet keine per-User MAM-Löschung via REST-API.
...
**Datei:** `ai_integration.vala`  
**Status:** ⚠️ Nicht fixbar — Google Gemini API erfordert den Key als URL-Query-Parameter (`?key=`). Alle anderen Provider verwenden Header-Auth. Kein Code-Fix möglich ohne API-Änderung seitens Google.
...
**Datei:** `bot_omemo.vala`  
**Status:** ⚠️ Offen — Workaround (`addr.device_id = 0`) an 5 Stellen noch vorhanden. C-Binding Lifecycle-Problem, low priority da funktional stabil.
...
**Datei:** `cert_gen.c`  
**Status:** ⚠️ Offen — `gnutls_global_init()` noch bei jedem Aufruf. Funktional harmlos (GnuTLS Reference-Counting), aber unsauber.
...
### BUG-22: `delete_dedicated_bot()` Fire-and-Forget ohne Fehlerbehandlung — TEILWEISE FIXED
**Datei:** `botfather_handler.vala`  
**Status:** ⚠️ Teilweise behoben — `.begin()` hat jetzt einen Callback der `.end(res)` aufruft und Erfolg/Fehler loggt. Jedoch: `registry.delete_bot()` läuft sofort weiter ohne auf das ejabberd-Ergebnis zu warten. Bei fehlgeschlagener Deregistrierung bleibt ein Zombie-Konto.

**Verbleibender Fix:** `delete_bot()` erst nach erfolgreichem Unregister ausführen, oder Retry-Liste für fehlgeschlagene Löschungen.
