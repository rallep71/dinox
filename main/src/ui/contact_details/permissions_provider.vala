...
public class PermissionsProvider : Plugins.ContactDetailsProvider, Object {
    ...
    public void populate(Conversation conversation, Plugins.ContactDetails contact_details, Plugins.WidgetType type) {
        ...
        if (stream_interactor.get_module<MucManager>(MucManager.IDENTITY).get_role(own_jid, conversation.account) == Xmpp.Xep.Muc.Role.VISITOR) {
            ...
            view_model.clicked.connect(()=> {
                stream_interactor.get_module<MucManager>(MucManager.IDENTITY).request_voice(conversation.account, conversation.counterpart);
            });
